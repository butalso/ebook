...
## Redis内存回收机制
Redis内存回收机制主要分为过期删除策略和内存淘汰策略。值得一提的是，Redis还采用了**引用计数算法**来跟踪对象的使用情况。尽管共享对象可以节约内存，但是权衡CPU时间的限制，Redis只对包含整数值的字符串对象进行共享。
### Redis过期策略
...
## Redis批处理pipeline
### 为什么使用pipeline
1. Redis工作过程采用的请求响应模式，意味着每一次请求，客户端都需要与服务端建立连接，发送命令，监听socket等待服务端处理结果。如果有大量的请求，客户端与服务端需要建立大量的TCP连接，等待网络传输命令请求与响应结果。这组命令耗时将会随着命令数量的增加而线性增加。
2. 一般来说，客户端都会采用连接池的方式保存与服务端的TCP连接，每次执行命令都会从池子里取出连接，用完之后再将连接放回池子。但这也只是节省了创建TCP连接的时间，在极端情况下，当我们需要有一组命令需要执行，同时池子里的连接被用完了，在指定的等待时间内如果没有等到空闲连接，则程序将出现异常。
3. 虽然池化连接可以减少建立连接的时间，但多条命令分别执行，将会执行多次socket网络IO，涉及read()、write()系统调用，从用户态到内核态，进行上下文切换，会消耗大量的CPU时间。

因此，我们需要一种占用极少连接，且不浪费请求、响应的往返时间，提高整体吞吐量的方式，pipeline。pipeline可以一次性发送多条命令并在执行完后一次性将结果返回，减少客户端与服务端通信的次数，提高整体吞吐量。
